...
function SignUp() {
  ...
  useEffect(() => {
    if(user === ''){
      navigate('/anmeldung', {state: location.state ? {background: location.state.background, auth: location.state.auth} : {}, replace: true});
    }
      ...
      content={
        <div>
          ...
          <div style={{paddingRight: "34px", paddingLeft: "34px", marginTop: '20px'}}>
            <Alert type={'user'} style={{marginBottom: '20px'}} error/>
            <Textfield
              label='Nutzername'
              value={username}
            ...
            <p style={{marginTop: '20px'}}>
              <Button variant="contained" sx={{borderRadius: 0, width: '100%'}} onClick={() => dispatch(register(username, password, email))}>Registrieren</Button>
            </p>
          </div>
          <Divider variant='fullWidth'/>
