...
function SignOut() {
  ...
  useEffect(() => {
    if(!user && last && (last + (Number(process.env.REACT_APP_API_TOKEN_EXPIRATION) * 1000 * 0.99) < Date.now())){
      setOpen(true);
    }
  }, [user, last]);
      ...
      content={
        <div>
          <div style={{paddingRight: "34px", paddingLeft: "34px", marginTop: '20px'}}>
            <div style={{textAlign: 'center', marginBottom: '16px'}}>Deine Sitzung ist abgelaufen. Du wurdest automatisch ausgeloggt.</div>
          </div>
          <Divider variant='fullWidth'/>
          <p style={{textAlign: 'center', paddingRight: "34px", paddingLeft: "34px", marginBottom: 0}}>
            Melde dich <StyledLink to="/anmeldung" onClick={onClose} state={{background: location.state && location.state.background ? location.state.background : location}} replace style={{fontWeight: 'bold'}}>hier</StyledLink> wieder an.
          </p>
        </div>
      }
    />
