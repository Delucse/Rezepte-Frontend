...
function TimePicker(props) {
    ...
    const [date, setDate] = useState(new Date(props.time + (new Date(props.time).getTimezoneOffset() * 60 * 1000)));
    const [previewDate, setPreviewDate] = useState(date);
    const [view, setView] = useState('hours');
    const [open, setOpen] = useState(false);

    useEffect(() => {
        setDate(new Date(props.time + (new Date(props.time).getTimezoneOffset() * 60 * 1000)));
      }, [props]);
    ...
    const submit = () => {
        var newDate = new Date(previewDate.getTime() - (new Date(previewDate.getTime()).getTimezoneOffset() * 60 * 1000))
