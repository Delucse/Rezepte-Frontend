...
function AddImage(props){
    ...
    const onHandleFileInput = async (targetFiles) => {
        ...
        if(targetFiles.length > 1){
            alert('Insgesamt zu viele Bilder. Es darf nur maximal ein Bilder hochgeladen werden.')
            return
        }
        ...
        user ? 
            <div>
                <Box sx={{color: theme => theme.palette.primary.main}}>
                    <Icon path={mdiCameraPlus} size={1} onClick={() => setOpen(true)} style={{cursor: 'pointer', padding: 8}}/>
                </Box>
                <Dialog 
                    open={open} 
                    ...
                    content={
                        <div>
                            ...
                            : 
                                <ImageListItem sx={{height: '180px', width: '100%'}}>
                                    <img src={image.url} alt='' style={{height: '180px', objectFit: 'cover'}}/>
                                    <ImageListItemBar
                                        actionIcon={
                                            <IconButton
                    ...
                    actions={
                        <div>
                            <Button variant="outlined" onClick={() => {setOpen(false); setImage(null);}} sx={{borderRadius: 0, mr: 1}}>
                                Abbrechen
                            </Button>
                            <Button disabled={image === null} variant="contained" onClick={submit} sx={{borderRadius: 0}}>
                                Bestätigen
                            </Button>
                        </div>
                    }    
